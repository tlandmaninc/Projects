...
class World:
    ...
    def plot(self,gamma, reward, theta):
        """
        plot function
        :return: None
        """
        ...
        for i in range(nCols):
            for j in range(nRows, 0, -1):
                ...
                k += 1
        plt.title('MDP Gridworld - Value Iteration\n\u03B3={0}, r: {1}, \u03B8: {2}'.format(gamma, reward, theta), size=16)
        plt.axis("equal")
        plt.axis("off")
        plt.show()
    ...
    def plot_value(self, valueFunction):
        ...
        for i in range(nCols):
            for j in range(nRows, 0, -1):
                ...
                k += 1
        plt.title('MDP Gridworld - Values', size=16)
        plt.axis("equal")
        plt.axis("off")
        plt.show()

    def plot_value2(self, valueFunction, type, gamma, reward, theta):
        ...
        for i in range(nCols):
            for j in range(nRows, 0, -1):
                ...
                k += 1
        plt.title('MDP Gridworld - {0} Iteration\n\n\u03B3={1}, r: {2}, \u03B8: {3}'.format(type,gamma, reward, theta), size=16)
        plt.axis("equal")
        plt.axis("off")
        plt.show()
    ...
    def plot_policy(self, policy):
        ...
        for i in range(nCols):
            for j in range(nRows, 0, -1):
                plt.text(i + 0.3, j - 0.3, str(states[k]), fontsize=16, horizontalalignment='right', verticalalignment='bottom')
                k += 1
        plt.title("MDP Gridworld - Best Policy")
        plt.axis("equal")
        plt.axis("off")
        plt.show()
        return plt.show()
